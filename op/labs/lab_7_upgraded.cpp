...
// конвертация месяца в номер месяца
...
// конвертация номера месяца в месяц
...
void loadData(struct deposit* deposits, int* global_counter) {
    ...
    for (int i = 0; i < counter; i++) {
        ...
        getline(file, s);
        number = std::stoi(s);
        deposits[i].creation_date.month = numToChar(number);
...
void openAccount(struct deposit* account, int number, double balance, int day, int month, int year) {
    ...
    char* str1 = (numToChar(number));
    account->creation_date.month = str1;
    ...
    while (status) {
        ...
        case 1:
            ...
            while (current_status) {
                ...
                int locall = (1 + ltm->tm_mon);
                current_date.month = numToChar(locall);
                ...
                openAccount(p_deposit, deposits_counter + 1, balance_input, current_date.day, monthToNum(current_date.month), current_date.year);
        ...
        case 5:
            ...
            while (current_status) {
                ...
                for (int i = 0; i < deposits_counter; i++) {
                    ...
                    if (isBigger(day_to_find, month_to_find, year_to_find, deposits[i].creation_date.day, monthToNum(deposits[i].creation_date.month), deposits[i].creation_date.year)) {
                        if (flag == 0) {
